...
export async function GET(req: NextRequest) {
  ...
  if (!session || !session.isAdmin) {
    console.log("LOG =====> Unauthorized GET /api/admin/clubs/:id");
...
export async function PUT(req: NextRequest) {
  try {
    const session = await verifySessionFromRequest(req);
    if (!session || !session.isAdmin) {
      console.log("LOG =====> Unauthorized PUT /api/admin/clubs/:id");
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
    const id = extractIdFromUrl(req);
    ...
    console.log(`LOG =====> Admin updated club ${updated.name}`);
...
export async function DELETE(req: NextRequest) {
  try {
    const session = await verifySessionFromRequest(req);
    if (!session || !session.isAdmin) {
      console.log("LOG =====> Unauthorized DELETE /api/admin/clubs/:id");
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
    const id = extractIdFromUrl(req);
    ...
    console.log(`LOG =====> Admin deleted club ${deleted?.name}`);
